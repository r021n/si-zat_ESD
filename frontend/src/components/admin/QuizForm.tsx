  ...
  return (
    <div className="w-full flex-1 flex flex-col justify-between overflow-hidden">
          ...
          {questions.map((q, qIdx) => (
            ...
            >
              <div className="flex justify-between items-center border-b border-[#F0EDFF]/50 pb-2">
                <span className="text-xs font-extrabold text-[#8C66FF] uppercase tracking-wider">
                  Soal #{qIdx + 1}
                </span>
                <button
                  type="button"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-black uppercase tracking-widest text-[#9C98A6]">
                  Teks Pertanyaan (Tinggi dapat diatur)
                </label>
                <textarea
                  value={q.text}
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-[#F0EDFF] text-xs bg-[#FAF9FF] text-[#2C2B30] font-medium focus:outline-none focus:border-[#8C66FF] focus:bg-white resize-y min-h-17.5 rounded-xl transition-none"
                />
              </div>
                ...
                {q.images && q.images.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-1.5">
                    {q.images.map((img, imgIdx) => (
                      <div
                        key={imgIdx}
                        className="relative w-14 h-14 rounded-xl border border-[#F0EDFF] bg-white flex items-center justify-center overflow-hidden shadow-sm"
                      >
                        <img
                          src={img}
                          alt="Preview"
              ...
              <div className="flex flex-col gap-2">
                <p className="text-[9px] font-black uppercase tracking-widest text-[#9C98A6] mb-0.5">
      ...
      <div className="w-full mt-6 mb-2">
        <button
          onClick={onSave}
          className="w-full py-4 bg-linear-to-br from-[#8C66FF] to-[#6039DF] text-white font-extrabold uppercase tracking-wider text-xs rounded-full shadow-md shadow-purple-100 cursor-pointer transition-none flex items-center justify-center gap-2"
        >
          Simpan Kuis
        </button>
      </div>
