  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center bg-white rounded-2xl overflow-hidden">
      <div className="relative max-h-full max-w-full aspect-9/16 flex items-center justify-center">
        ...
        {/* Overlay interactive content inside the white space of 17_polos.png */}
        <div
          className="absolute flex flex-col justify-between overflow-y-auto text-left p-2 sm:p-2.5 text-[#2C2B30] z-10 custom-scrollbar"
          style={{
            top: "26%",
            left: "11%",
            width: "78%",
        ...
        >
          <div className="space-y-2">
            {/* Question title */}
            <p className="text-[10.5px] sm:text-xs font-bold text-[#1E293B] leading-snug">
              Manakah Tindakan yang Mendukung Pelestarian Lingkungan melalui
              ...
              {PAGE17_OPTIONS.map((opt) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[9px] sm:text-[10px] leading-tight flex-1 font-medium">
                      {opt.text}
                    </span>
                  </button>
                );
          ...
          {isSubmitted && (
            <div className="mt-2 bg-white border-2 border-[#1E3A8A] rounded-xl p-2 sm:p-2.5 shadow-md flex flex-col justify-between animate-fade-in shrink-0">
              <div>
                <div className="flex items-center justify-between border-b border-blue-100 pb-1 mb-1 shadow-2xs">
                  <div className="flex items-center gap-1.5 text-[#1E3A8A]">
                    <BookOpenIcon size={14} className="shrink-0" />
                    <span className="font-extrabold text-xs tracking-wide">
                      Pembahasan
                    </span>
                  </div>
                  <span className="bg-[#1E3A8A] text-white font-black text-[9px] px-2 py-0.5 rounded-md shadow-xs">
                    Jawaban B
                  </span>
                </div>
                <p className="text-[8.5px] sm:text-[9.5px] text-[#1E293B] font-medium leading-relaxed">
                  Menjemur pakaian dengan memanfaatkan panas sinar matahari merupakan
                  contoh pemanfaatan perubahan fisika (penguapan air) yang mendukung
                  pelestarian lingkungan karena menghemat penggunaan energi listrik
                  dibandingkan mesin pengering.
                </p>
              </div>

              <div className="pt-1.5 mt-1 border-t border-blue-50 flex justify-between items-center text-[9px] sm:text-[10px]">
                <span
                  className={`font-bold ${
                    selectedOption === "B"
                      ? "text-emerald-700"
                ...
                <button
                  type="button"
                  onClick={onReset}
                  className="px-2.5 py-1 bg-[#F0ECFF] text-[#8C66FF] border border-[#DCD6F7] hover:bg-purple-100 font-extrabold rounded-lg cursor-pointer transition-all flex items-center gap-1 text-[9px] active:scale-95"
                >
